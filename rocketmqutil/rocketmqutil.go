...
func InitRocketmqutil(confMap map[string][]string) {
...
func _init() {
	confMap := confutil.GetConfArrayMap("RocketmqProxy")
	if unixs, ok := confMap["unix"]; ok {
		if len(unixs) > 0 {
			uSocket = confMap["unix"][0]
		}
...
func sendBySocket(msg []byte) (back string, err error) {
	...
	message := socket.GetMessage(socket.WithSetMeta("X-MQTYPE", "rocketmq"))
...
func Send2Proxy(topic, tags string, msg []byte, keys ...string) error {
	var err error
	...
	if tags == "" || strings.Contains(tags, " ") {
		err = errors.New("tags can not be empty or contain empty")
		logger.E("TagsError", "Tags err:%v", err)
		return err
	}
